...
get_context_percentage() {
    ...
    if [ -d "$projects_dir" ]; then
        # FIRST: Try to use the exact session_id provided by this Claude Code instance
        if [ -n "$session_id" ] && [ "$session_id" != "test" ]; then
            jsonl_file=$(find "$projects_dir" -name "${session_id}.jsonl" -type f 2>/dev/null | head -1)
        fi

        # FALLBACK: If exact session not found, use the most recent session file in this project
        # This handles forked chats where the new session_id won't match the original
        if [ -z "$jsonl_file" ] || [ ! -f "$jsonl_file" ]; then
            if [ "$DEBUG" = "1" ]; then
                echo "DEBUG: Exact session not found, looking for most recent session file..." >&2
            fi
            # Find the most recently modified .jsonl file in this project directory
            jsonl_file=$(find "$projects_dir" -name "*.jsonl" -type f -exec stat -f "%m %N" {} \; 2>/dev/null | sort -nr | head -1 | cut -d' ' -f2-)
            if [ "$DEBUG" = "1" ] && [ -n "$jsonl_file" ]; then
                echo "DEBUG: Using most recent session file: $jsonl_file" >&2
            fi
        fi
    else
        ...
        # Ultimate fallback: most recent session file in any project directory if we're in a subdirectory
        if [ -z "$jsonl_file" ] || [ ! -f "$jsonl_file" ]; then
            # Check if current directory is a subdirectory of any existing project
            for proj_dir in "$projects_base"/*; do
                if [ -d "$proj_dir" ]; then
                    local proj_path=$(echo "${proj_dir##*/}" | sed 's/^-//' | sed 's/-/\//g')
                    if [[ "$cwd" == *"$proj_path"* ]]; then
                        jsonl_file=$(find "$proj_dir" -name "*.jsonl" -type f -exec stat -f "%m %N" {} \; 2>/dev/null | sort -nr | head -1 | cut -d' ' -f2-)
                        if [ "$DEBUG" = "1" ] && [ -n "$jsonl_file" ]; then
                            echo "DEBUG: Found related project session file: $jsonl_file" >&2
                        fi
                        break
                    fi
                fi
...
case $MONTH in
    01|02|03) QUARTER="Q1"; QUARTER_START_MONTH=1 ;;
    04|05|06) QUARTER="Q2"; QUARTER_START_MONTH=4 ;;
    07|08|09) QUARTER="Q3"; QUARTER_START_MONTH=7 ;;
    10|11|12) QUARTER="Q4"; QUARTER_START_MONTH=10 ;;
esac
...
# Line 2: Current directory | Git branch
